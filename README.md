...
| C04 | Validar headers da resposta | • `Content-Type: application/json`<br>• Verificar vazamento de dados sensíveis nos headers<br>• Verificar presença/ausência de headers de `Cache-Control` | PASS |
| C05 | Validar parâmetro `limit` | • Retorno deve ser igual ao valor de `limit` enviado<br>• Limites válidos: `1` a `20`<br>• Limites inválidos: `0`, `-1`, `abc` | VÁLIDOS-PASS + INVÁLIDOS-DOCUMENTADO EM ACHADOS|
